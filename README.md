...
The following table lists runtime with maximum length of 11 on an average
laptop:
...
and store those in the caches. However, at the end of the run with `kMaxLength =
11`, the related cache has only 20 entries, and the not related cache has only
7,590,294 entries (compared to 17,592,186,044,416 possible pairs of combinations
of 11 character length each drawn from 4 letters). The related cache does not
receive a single hit, and the not related cache has a hit rate of
$1.2\cdot10^{-6}$. So no optimization is likely to make memoization worth it.
(The timing result above was measured with cache hit and miss recording disabled
...
Surprisingly, the construct algorithm turns out to take roughly the same amount
of time as the recursive one. By the end of the run it finds a total of
1,018,023 pairs of specially related combinations of length at most 11. On the
one hand, construction means deeper call stacks than recursion, about a firstful
of steps from each starting point for construct as opposed to very few in the
typical case for recursion, also construction involves string copies as opposed
to passing around Views by recursion. On the other hand, the recursive algorithm
has to examine all possible combinations up to length 11, while construction
only explores starting from specially related pairs generated by the Q property
over all possible combinations up to length 9 (so that even with the Q character
